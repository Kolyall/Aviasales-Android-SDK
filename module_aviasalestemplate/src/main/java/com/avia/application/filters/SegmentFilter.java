...
public class SegmentFilter implements Serializable {
    ...
    @Getter @Setter private final AllianceFilter allianceFilter;
    @Getter @Setter private final AirlinesFilter airlinesFilter;
    @Getter @Setter private final AirportsFilter airportsFilter;
    @Getter @Setter private final BaseNumericFilter durationFilter;
    @Getter @Setter private final BaseNumericFilter stopOverDelayFilter;
    @Getter @Setter private final BaseNumericFilter landingTimeFilter;
    @Getter @Setter private final BaseNumericFilter takeoffTimeFilter;
    @Getter @Setter private final BaseNumericFilter stopOverCountFilter;
    @Getter @Setter private final OvernightFilter overnightFilter;
    ...
    public boolean isSuitedByStopOverDelay(List<Flight> flights) {
        if (stopOverDelayFilter.isActive()) {
            ...
            return !(!stopOverDelayFilter.isActualForMaxValue(directMinMaxStopOverDelay.get(TicketData.MAX)) || !stopOverDelayFilter.isActualForMinValue(directMinMaxStopOverDelay
                    .get(TicketData.MIN)));
    ...
    public void initMinMaxValues(List<Flight> flights, Map<String, AirportData> airportDataMap, Map<String, AirlineData> airlineDataMap) {
        int departureInMinutesFromDayBeginning = flights.get(0).getDepartureInMinutesFromDayBeginning();
        takeoffTimeFilter.setMinValue(Math.min(takeoffTimeFilter.getMinValue(), departureInMinutesFromDayBeginning));
        takeoffTimeFilter.setMaxValue(Math.max(takeoffTimeFilter.getMaxValue(), departureInMinutesFromDayBeginning));

        int arrivaInMinutesFromDayBeginning = flights.get(0).getArrivaInMinutesFromDayBeginning();
        landingTimeFilter.setMinValue(Math.min(landingTimeFilter.getMinValue(), arrivaInMinutesFromDayBeginning));

        landingTimeFilter.setMaxValue(Math.max(landingTimeFilter.getMaxValue(), arrivaInMinutesFromDayBeginning));
    ...
    public Map<String, Integer> getDirectMinAndMaxStopOverDurationInMinutes(List<Flight> flights) {
        ...
        for (int i = 1; i < flights.size(); i++) {
            minMaxStopOverDurationHashMap.put(MIN, Math.min(minMaxStopOverDurationHashMap.get(MIN), flights.get(i)
                    .getDepartureInMinutes()
                    .intValue() - flights.get(i - 1).getArrivalInMinutes().intValue()));
            minMaxStopOverDurationHashMap.put(MAX, Math.max(minMaxStopOverDurationHashMap.get(MAX), flights.get(i)
                    .getDepartureInMinutes()
                    .intValue() - flights.get(i - 1).getArrivalInMinutes().intValue()));
        }
