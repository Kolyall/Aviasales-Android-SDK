...
public class OpenJawFiltersSet implements Serializable, FiltersSet {
    ...
    private boolean shouldAddTicketToResults(Map<String, AirlineData> airlines, List<ResultsSegment> resultsSegments, Proposal proposal) {
        return isSuitedByPrice(proposal) &&
    ...
    public boolean isSuitedBySegmentFilters(Map<String, AirlineData> airlines, Proposal proposal) {
        for (int i = 0; i < proposal.getSegments().size(); i++) {
            ...
            if (segmentFilter.isActive()) {
                if (!segmentFilter.isSuitedByAirline(proposal.getSegmentFlights(i)) || !segmentFilter.isSuitedByAirport(proposal.getSegmentFlights(i)) || !segmentFilter
                        .isSuitedByAlliance(airlines, proposal.getSegmentFlights(i)) || !segmentFilter.isSuitedByDuration(proposal
						.getSegmentFlights(i)) || !segmentFilter
                        .isSuitedByOvernight(proposal.getSegmentFlights(i)) || !segmentFilter.isSuitedByStopOver(proposal) || !segmentFilter
                        .isSuitedByStopOverDelay(proposal.getSegmentFlights(i)) || !segmentFilter.isSuitedByTakeoffTime(proposal
						.getSegmentFlights(i)) || !segmentFilter
                        .isSuitedByLandingTime(proposal.getSegmentFlights(i))) {
                    return false;
                }
            }
        }
    ...
    public synchronized void initMinAndMaxValues(Context context, SearchData searchData, List<Proposal> proposals) {
        ...
        for (Proposal proposal : proposals) {
            ...
            for (int segmentId = 0; segmentId < proposal.getSegments().size(); segmentId++) {
                ...
                segmentFilters.get(segmentId)
                        .initMinMaxValues(proposal.getSegments().get(segmentId).getFlights(), airportDataMap, airlineDataMap);
            }
        }
