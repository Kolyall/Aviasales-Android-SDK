...
import com.avia.application.utils.FiltersUtils;
...
public class SimpleSearchFilters implements Serializable, FiltersSet {
    @Getter private final AgenciesFilter agenciesFilter;
    @Getter private final BaseNumericFilter priceFilter;
    @Getter private final PayTypeFilter payTypeFilter;
    @Getter private final BaseNumericFilter durationFilter;
    @Getter private final BaseNumericFilter stopOverDelayFilter;
    @Getter private final BaseNumericFilter takeoffTimeFilter;
    @Getter private final BaseNumericFilter takeoffBackTimeFilter;
    @Getter private final BaseNumericFilter landingTimeFilter;
    @Getter private final BaseNumericFilter landingBackTimeFilter;
    @Getter private final BaseNumericFilter stopOverSizeFilter;
    @Getter private final OvernightFilter overnightFilter;
    @Getter private final AllianceFilter allianceFilter;
    @Getter private final AirlinesFilter airlinesFilter;
    @Getter private final AirportsFilter airportsFilter;
    ...
    private boolean shouldAddTicketToResults(Map<String, AirlineData> airlines, List<ResultsSegment> segments, Proposal proposal) {
    ...
    public boolean isSuitedByLandingTime(Proposal proposal) {
        if (landingTimeFilter.isActive()) {
            Calendar calendar = Calendar.getInstance();
            int landingTime;
            calendar.setTimeInMillis(proposal.getSegmentFlights(0)
                    .get(proposal.getSegmentFlights(0).size() - 1)
                    .getLocalArrivalTimestamp() * 1000);
            calendar.setTimeZone(TimeZone.getTimeZone("UTC"));
    ...
    public boolean isSuitedByLandingBackTime(Proposal proposal) {
        if (landingBackTimeFilter.isActive()) {
            Calendar calendar = Calendar.getInstance();
            int landingBackTime;
            calendar.setTimeInMillis(proposal.getSegmentFlights(1)
                    .get(proposal.getSegmentFlights(1).size() - 1)
                    .getLocalArrivalTimestamp() * 1000);
            calendar.setTimeZone(TimeZone.getTimeZone("UTC"));
    ...
    public boolean isSuitedByStopOverDelay(Proposal proposal) {
        if (stopOverDelayFilter.isActive()) {
            ...
            if (!stopOverDelayFilter.isActualForMaxValue(directMinMaxStopOverDelay.get(Proposal.MAX)) || !stopOverDelayFilter.isActualForMinValue(directMinMaxStopOverDelay
                    .get(Proposal.MIN))) {
                return false;
            }
            ...
            if (proposal.getSegments().size() == 2) {
                ...
                if (!stopOverDelayFilter.isActualForMaxValue(returnMinMaxStopOverDelay.get(Proposal.MAX)) || !stopOverDelayFilter
						.isActualForMinValue(returnMinMaxStopOverDelay
                        .get(Proposal.MIN))) {
                    return false;
                }
            }
    ...
    public boolean isSuitedByDuration(Proposal proposal) {
        if (durationFilter.isActive()) {
            ...
            return durationFilter.isActual(directDuration) && durationFilter.isActual(returnDuration == 0 ? directDuration :
					returnDuration);
        } else {
            return true;
        }
    ...
    public synchronized void initMinAndMaxValues(Context context, SearchData searchData, List<Proposal> proposals) {
        ...
        for (Proposal proposal : proposals) {
            ...
            takeoffTimeFilter.setMinValue(Math.min(takeoffTimeFilter.getMinValue(), proposal.getSegmentFlights(0)
                    .get(0)
                    .getDepartureInMinutesFromDayBeginning()));
            takeoffTimeFilter.setMaxValue(Math.max(takeoffTimeFilter.getMaxValue(), proposal.getSegmentFlights(0)
                    .get(0)
                    .getDepartureInMinutesFromDayBeginning()));
            landingTimeFilter.setMinValue(Math.min(landingTimeFilter.getMinValue(), proposal.getSegmentFlights(0)
                    .get(proposal.getSegmentFlights(0).size() - 1)
                    .getArrivaInMinutesFromDayBeginning()));
            landingTimeFilter.setMaxValue(Math.max(landingTimeFilter.getMaxValue(), proposal.getSegmentFlights(0)
                    .get(proposal.getSegmentFlights(0).size() - 1)
                    .getArrivaInMinutesFromDayBeginning()));

            if (proposal.getSegments().size() >= 2) {
                takeoffBackTimeFilter.setMinValue(Math.min(takeoffBackTimeFilter.getMinValue(), proposal.getSegmentFlights(1)
                        .get(0)
                        .getDepartureInMinutesFromDayBeginning()));
                takeoffBackTimeFilter.setMaxValue(Math.max(takeoffBackTimeFilter.getMaxValue(), proposal.getSegmentFlights(1)
                        .get(0)
                        .getDepartureInMinutesFromDayBeginning()));
                landingBackTimeFilter.setMinValue(Math.min(landingBackTimeFilter.getMinValue(), proposal.getSegmentFlights(1)
                        .get(proposal.getSegmentFlights(1).size() - 1)
                        .getArrivaInMinutesFromDayBeginning()));
                landingBackTimeFilter.setMaxValue(Math.max(landingBackTimeFilter.getMaxValue(), proposal.getSegmentFlights(1)
                        .get(proposal.getSegmentFlights(1).size() - 1)
                        .getArrivaInMinutesFromDayBeginning()));
            }
            ...
            if (proposal.getSegments().size() >= 2) {
                ...
                stopOverDelayFilter.setMaxValue(Math.max(stopOverDelayFilter.getMaxValue(), stopOverMinMaxReturnDuration.get(Proposal
						.MAX)));
                stopOverDelayFilter.setMinValue(Math.min(stopOverDelayFilter.getMinValue(), stopOverMinMaxReturnDuration.get(Proposal
						.MIN)));
            }
            ...
            if (!overnightFilter.isAirportOvernightViewEnabled() && overnightFilter.hasOvernight(proposal.getSegmentFlights(0))) {
                overnightFilter.setAirportOvernightViewEnabled(true);
            }
