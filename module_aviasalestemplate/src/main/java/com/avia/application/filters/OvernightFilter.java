...
public class OvernightFilter implements Serializable {
    ...
    @Getter @Setter private boolean isAirportOvernightAvailable = true;

    @Getter @Setter private boolean isAirportOvernightViewEnabled = false;

    @Getter @Setter private List<OvernightTerms> overnightTermsList;
    ...
    public boolean isActual(List<Flight> flights) {
        for (int i = 0; i < flights.size(); i++) {
            for (OvernightTerms overnightTerms : overnightTermsList) {
                if ((!isAirportOvernightAvailable && i != 0 && overnightTerms.isOvernight(flights.get(i)
                        .getDepartureInMinutes() - flights.get(i - 1).getArrivalInMinutes(), flights.get(i - 1)
                        .getArrivalInHoursFromDayBeginning()))) {
                    return false;
                }
            }
        }
    ...
    public boolean hasOvernight(List<Flight> flights) {
        for (int i = 0; i < flights.size(); i++) {
            for (OvernightTerms overnightTerms : overnightTermsList) {
                if ((i != 0 && overnightTerms.isOvernight(flights.get(i).getDepartureInMinutes() - flights.get(i - 1)
                        .getArrivalInMinutes(), flights.get(i - 1).getArrivalInHoursFromDayBeginning()))) {
                    return true;
                }
            }
        }
